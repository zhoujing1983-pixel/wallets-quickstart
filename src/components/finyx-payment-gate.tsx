...
export function PaymentGate({
  ...
  const walletAddress = emailWallet?.address ?? wallet?.address ?? "";
  const ownerLabel = useMemo(() => {
    ...
    if (wallet?.owner) {
      return String(wallet.owner).replace(/^[^:]*:/, "");
    }
    return "Current User";
  }, [emailWallet?.locator, emailWallet?.owner, wallet?.owner]);
  const walletChain =
    emailWallet?.chain ??
  ...
  const resolveOtpPromise = (value: boolean) => {
    if (otpPromiseRef.current) {
      otpPromiseRef.current.resolve(value);
      otpPromiseRef.current = null;
    }
  };

  const ensureOtpAuthorized = async () => {
    if (otpAuthorized) return true;
    if (!otpPromiseRef.current) {
      openOtpModal();
      ...
      otpPromiseRef.current = { promise, resolve };
    }
    const currentPromise = otpPromiseRef.current?.promise;
    return currentPromise ?? false;
  };
  ...
  const buildMemoData = (signature: string | null) =>
    signature && signature !== "otp_verified" ? { memo_tx: signature } : {};

  const resetOtpSession = async () => {
    if (isResettingSession) {
      return;
    }
    setIsResettingSession(true);
    try {
      await fetch("/api/auth/email/transfer/otp/reset", { method: "DELETE" });
    } catch (error) {
  ...
  const signWithOtp = async (): Promise<string | null> => {
    const authorized = await ensureOtpAuthorized();
    if (!authorized) {
      reportIntentFlowError("Email OTP authorization required.");
      return null;
    }
    return "otp_verified";
  };
  ...
  const signIntent = async (messageOverride?: string | null) => {
    ...
    const signature = await signWithOtp();
  ...
  const verifyIntentSignature = async (
    ...
    if (!token || (!signature && !otpAuthorized)) {
  ...
  const mintDelegationToken = async (
    ...
    if (!challenge || (!signature && !otpAuthorized)) {
  ...
  const buildExplorerUrl = (txId: string) => {
    if (!String(walletChain).toLowerCase().includes("solana")) {
      return null;
    }
    return `https://explorer.solana.com/tx/${txId}?cluster=${memoCluster}`;
  };

  const pollTransferStatus = async (transactionId: string) => {
    let attempts = 0;
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (transactionId) {
        const polled = await pollTransferStatus(String(transactionId));
        if (polled?.txId) {
          setTxHash(polled.txId);
          setTxHashInput(polled.txId);
          if (polled.explorer) {
            setExplorerLink(polled.explorer);
          }
          return polled.txId;
        }
      }
      reportIntentFlowError("Waiting for transfer confirmation.");
      return null;
    } catch (error) {
    ...
    try {
      ...
      setStepResults((prev) => [
        ...prev,
        {
          flow: "A",
          step: "自动授权 / 交互授权",
          data: { signature, ...buildMemoData(signature) },
          timestamp: Date.now(),
        },
      ]);
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-col items-start gap-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  {isResettingSession ? "重置中..." : "重置 OTP"}
                </button>
                <Link
                  href="/finyx/dashboard"
          ...
          <div className="mt-2 text-xs text-slate-600 space-y-2">
            ...
            {emailWalletError ? (
              <div className="text-rose-600">
                Email wallet error: {emailWalletError}
              </div>
            ) : null}
      ...
      <TransferOtpModal
        open={otpOpen}
        step={otpStep}
        email={otpEmail ?? emailAddress}
        code={otpCode}
        resendSeconds={otpResendSeconds}
        isSubmitting={otpIsSubmitting}
        error={otpError}
        introTitle="Authorize this action"
        introDescription="We need a one-time authorization code to continue. Click 'Send code' to receive it."
        onClose={() => {
          setOtpOpen(false);
          setOtpError(null);
          resolveOtpPromise(false);
        }}
